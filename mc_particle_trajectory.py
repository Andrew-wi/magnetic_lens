...
bxMatrix = np.array([hdulBxMatrix[0].data[0], hdulBxMatrix[0].data[1], \
    hdulBxMatrix[0].data[2]])
byMatrix = np.array([hdulByMatrix[0].data[0], hdulByMatrix[0].data[1], \
    hdulByMatrix[0].data[2]])
bzMatrix = np.array([hdulBzMatrix[0].data[0], hdulBzMatrix[0].data[1], \
    hdulBzMatrix[0].data[2]])
...
bxDataDraft = [[i] * m for i in np.linspace(-R/2, R/2, m)]
byDataDraft = [[i] * m for i in np.linspace(-R/2, R/2, m)]
# Todo: adjust bzDataDraft to include the additional appended 0's
bzDataDraft = [[i] * m for i in np.linspace(-R/2, R/2, m)]

# Flatten lists
bxData = [item for sublist in bxDataDraft for item in sublist]
byData = [item for sublist in byDataDraft for item in sublist]
# Todo: adjust bzData to include the additional appended 0's
bzData = [item for sublist in bzDataDraft for item in sublist]

gradBMatrixAx.scatter3D(bxData, byData, bzData, cmap='Greens')
plt.show()
