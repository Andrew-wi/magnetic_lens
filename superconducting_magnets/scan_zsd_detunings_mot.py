...
mot_region = np.array([250, 500, 750, 1000, 1250, 1500]) # units: mm
results_dict_list = []
initial_nodecel_list = []

# initial population runs
for i, z_len in enumerate(mot_region):
    ...
    print(f'Successes, no decel, z_len = {mot_region[i] / 1e3}: {succ_pp}')
    print(f'Successful particles, no decel, desired velocity class, z_len = ' + \
          f'{mot_region[i] / 1e3}: {succ_pp_desired}')
...
# change if 1m is no longer at index 3 in mot_region
...
for i, del_s2w in enumerate(del_0_s2w_list):
    ...
    for j, z_len in enumerate(mot_region):
